...
func _spawn_part(part: BuildingPart, y_offset: float) -> float:
	...
	for child in instance.get_children():
		if child is MeshInstance3D:
			child.set_instance_shader_parameter("main_color", main_color)
			child.set_instance_shader_parameter("secondary_color", secondary_color)
	...
	for child in get_children():
		...
		for child2 in child.get_children():
			if child2 is MeshInstance3D:
				child2.set_instance_shader_parameter("main_color", main_color)
				child2.set_instance_shader_parameter("secondary_color", secondary_color)
